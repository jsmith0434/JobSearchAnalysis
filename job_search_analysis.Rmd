...
Sys.setenv("plotly_username"="jsmith0434")
Sys.setenv("plotly_api_key"="4B9ZcPrnvz92oO9fC0Bs")
```
...
final_table <- contingency_df %>%
  ...
  mutate(success_rate = round(success_rate * 100, 2))  # Convert success rate to percentage
...
no_response_count <- sum(data$response.date=="")
no_response_count
```
...
# Plot with totals above the bars, descending order, and more interesting colors
p4 <- ggplot(job_distribution, aes(x = reorder(job_category, -count), y = count, fill = job_category)) +
  geom_bar(stat = "identity", show.legend = FALSE) +  # Remove the legend
  geom_text(aes(label = count), vjust = -0.5, size = 4) +  # Add totals above the bars
  scale_fill_manual(values = my_colors) +  # Apply the chosen color palette
  labs(title = "Distribution of Applications by Job Title", x = "Job Category", y = "Number of Applications") +
...
ggplotly(p4)
#api_create(p4, filename = "Applications by Job Title")
```
...
# Bar chart of success rates by job category
p5 <- ggplot(normalized_interview_data, aes(x = reorder(job_category, success_rate), y = success_rate, fill = job_category)) +
  geom_bar(stat = "identity", color = "white", show.legend = FALSE) +  # Bars with success rate
  geom_text(aes(label = paste0(round(success_rate * 100, 1), "%")), vjust = 1, size = 4) +  # Add percentage labels above the bars
...
ggplotly(p5)
#api_create(p5, filename = "Success rate by Job Title")
...
# Bubble plot with custom colors and no gray background
p6 <- ggplot(normalized_interview_data, aes(x = reorder(job_category, success_rate), y = success_rate, size = self_assessment, color = job_category)) +
    geom_point(alpha=1) +
   # scale_size(range = c(5, 18), name="Self-Assessment Rating") + # Show size legend for Self-Assessment
    scale_color_manual(values = my_colors, guide = "none") + # Apply custom colors but hide the color legend
    scale_y_continuous(labels = scales::percent_format(scale = 100), limits = c(0, .10)) + # Set y-axis to percent with limits from 0 to 10%
    theme_minimal() + # Change background to minimal (white)
    ...
    theme(legend.position = "none",
          plot.caption = element_text(hjust = 0.1), # Center the caption
          plot.caption.position = "plot") +  # Remove legend 
    scale_size_continuous(range=c(5,15))

ggplotly(p6)
#api_create(p6, filename = "Success rate by Job Title and Self Assessment")
```
...
cleaned_data <- remove_outliers(data2$job.post.age)
